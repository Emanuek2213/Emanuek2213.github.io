...
class ajaxVentaSolicitud{

    
    
    public function AjaxGenerarSolicitud($nro_boleta, $arrIndicador, $arrPrecios , $arrCliente)
    {
        ...
        $this->generarSolicitud($pdf, $nro_boleta, $arrIndicador, $arrPrecios,$arrCliente, 4, 4);

        // Generar la segunda nota de venta en la nueva página
        $this->generarSolicitud($pdf, $nro_boleta, $arrIndicador, $arrPrecios, $arrCliente, 152, 4);
    ...
    private function generarSolicitud($pdf, $nro_boleta, $arrIndicador, $arrPrecios,$arrCliente, $x, $y)
    {   
        $pdf->SetXY($x, $y);
        ...
        $pdf->Text($x+ 104, $posY + 6, 'NOTA DE SOLICITUD');
    ...
    // {
    //    // Establecer posición inicial para elementos
    //     $pdf->SetXY($x, $y);

    //     // Título
    //     $pdf->SetFont('Arial', 'B', 16);
    //     $pdf->Rect($x+ 97, $pdf->GetY(), 38, 16);
    ...
    //     // Restaurar la posición original después de escribir el texto "INVERSIONES JUSYBA SAC"
    //     $pdf->SetXY($x, $posY + 10); // Sumar 10 para dar un espacio entre el texto y el rectángulo
    ...
    //     $pdf->SetFont('Arial', 'B', 9);
    //     $pdf->SetXY(107, $posY); // Utilizar la posición original de Y del rectángulo
    //     $pdf->Text($x+ 99, $posY + 6, 'NOTA DE COTIZACION');
    //     $pdf->SetFont('Arial', 'B', 11);
    //     $pdf->Text($x + 102, $posY + 13, '0001-'.$nro_boleta);
    ...
    //     $pdf->SetFont('Arial', '', 9);
    //     $pdf->Rect($x+0, $pdf->GetY(), 135, 14);
    //     $pdf->Text($x+3, $posY + 27, 'Sr(es) : '.utf8_decode($DatosCliente[1]));
    //     $pdf->Text($x+3, $posY + 31, 'RUC : '.$DatosCliente[0]);
    //     $pdf->Text($x+3, $posY + 35, 'Direccion : '.$DatosCliente[2]);
    ...
    //     $pdf->Cell(9, 6,'num', 1, 0, 'C');
    //     $pdf->Cell(9, 6, 'Cant', 1, 0, 'L');
    //     $pdf->Cell(14, 6, 'UM', 1, 0, 'C');
    //     $pdf->Cell(18, 6, 'Codigo', 1, 0, 'C');
    //     $pdf->Cell(57, 6, 'Descripcion', 1, 0, 'L');
    //     $pdf->Cell(14, 6, 'Precio', 1, 0, 'L');
    //     $pdf->Cell(14, 6, 'Total', 1, 1, 'L');

    //     $productos = VentasControlador::ctrObtenerDetalleCotizacion($nro_boleta);
    ...
    //         preg_match('/^(\d+)(.*)$/', $nuevoArray[$cantidad], $matches);
    //         $number = $matches[1]; // Aquí obtienes el número (en este caso, "1")
    //         $text = $matches[2];
    ...
    //         $pdf->Cell(9, 5, $numero.'.-', 1, 0, 'L', $fill);
    //         $pdf->SetFont('Arial', '', 7);
    //         $pdf->Cell(9, 5, number_format($number,2), 1, 0, 'C', $fill);
    //         $pdf->Cell(14, 5, $text, 1, 0, 'C', $fill);
    //         $pdf->Cell(18, 5, $pro["codigo_producto"], 1, 0, 'C', $fill);
    //         $pdf->SetFont('Arial', '', 7);
    //         $pdf->Cell(57, 5, utf8_decode(substr($pro["descripcion_producto"], 0, 35)), 1, 0, 'L', $fill);
    //         $pdf->SetFont('Arial', '', 8);
    //         $pdf->Cell(14, 5, $nuevoArray2[$cantidad], 1, 0, 'L', $fill);
    //         $pdf->Cell(14, 5, "S./ " . number_format($pro["cantidad"] * $pro["precio_unitario_venta"], 2), 1, 1, 'L', $fill);
    //         $cantidad_filas += 5;
    //         $numero += 1;
    //         $cantidad += -1;
    ...
    //     $decenas = array('', 'DIEZ', 'VEINTE', 'TREINTA', 'CUARENTA', 'CINCUENTA', 'SESENTA', 'SETENTA', 'OCHENTA', 'NOVENTA');
    //     $unid_dec = array('ONCE', 'DOCE', 'TRECE', 'CATORCE', 'QUINCE', 'DIECISEIS', 'DIECISIETE', 'DISECIOCHO', 'DIECINUEVE');
    //     $centenas = array('', 'CIENTO', 'DOSCIENTOS', 'TRESCIENTOS', 'CUATROCIENTO', 'QUINIENTOS', 'SEISCIENTOS', 'SETECIENTOS', 'OCHOCIENTOS', 'NOVECIENTOS');
    //     $miles = array('', 'MIL', 'DOS MIL', 'TRES MIL', 'CUATRO MIL', 'CINCO MIL', 'SEIS MIL', 'SIETE MIL', 'OCHO MIL', 'NUEVE MIL', 'DIEZ MIL');
    //     $centavos = array('CERO', 'DIEZ', 'VEINITE', 'TREINTA', 'CUARENTA', 'CINCUENTA', 'SESENTA', 'SETENTA', 'OCHENTA', 'NOVENTA');
    ...
    //     $texto .= ' SOLES';
    //     if ($decimal > 0) {
    //         $texto .= ' Y ' . $centavos[$decimal] . ' CENTIMOS';
    //     }
    ...
    //     $pdf->SetFillColor(220);
    //     $pdf->Rect($x , $pdf->GetY(), 135, 15, 'F');
    //     $pdf->Rect($x , $pdf->GetY(), 135, 15);
        

    //     $pdf->SetFont('Arial', 'B', 9);
    //     $pdf->Text($x + 3,  195, 'SON : '. $texto);
    //     $pdf->Text($x + 102,  199, 'TOTAL : S./ '. number_format($suma_total,2));
...
if (isset($_GET["nro_boleta"]) && isset($_GET["arrIndicador"])&& isset($_GET["arrPrecios"])) {
    ...
    $ventas = new ajaxVentaSolicitud();
    $ventas->AjaxGenerarSolicitud($_GET["nro_boleta"], $_GET["arrIndicador"], $_GET["arrPrecios"], $_GET["arrCliente"]);
}